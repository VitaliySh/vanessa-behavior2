...
Процедура ЗаписатьВыполнениеШагаАллюр2(МассивШагов, ДеревоТестов, РезультатыПрохождения, СоответствиеГрупп, МассивXMLОтчетаAllure)
	...
	Если РезультатыПрохождения.ВремяНачала <> Неопределено Тогда
		Шаг.start = РезультатыПрохождения.ВремяНачала;
		Шаг.stop  = РезультатыПрохождения.ВремяОкончания;
	КонецЕсли;
	Шаг.stage = "finished";
	...
	Если СтрокаВыполнения <> Неопределено Тогда	
		...
		Если СтрокаВыполнения.ШагСПараметрамиВТаблице = Истина Тогда
			...
			Если МассивПараметров.Количество() > 0 Тогда
				РеальноеИмяФайла = Строка(Новый УникальныйИдентификатор) + "-attachment.json";
				//ЗаписатьОбъектВJson(МассивПараметров, РеальноеИмяФайла, МассивXMLОтчетаAllure);
				//ДобавитьФайлКШагуАллюр2(Шаг, "table", РеальноеИмяФайла, "application/json");
		...
		КонецЕсли;
		///
		//Если СтрокаВыполнения.ШагСПараметрамиВТаблице Тогда
		//	Счетчик = 1;
		//	Для каждого СтрокаПараметра из СтрокаВыполнения.Строки Цикл
		//		ИмяПараметра = ""+Счетчик;
		//		Шаг["parameters"].Добавить(Новый Структура("name, value", Строка(Счетчик), СтрокаПараметра.Имя));
		//		Счетчик = Счетчик + 1;
		//	КонецЦикла;
		//КонецЕсли;
	КонецЕсли;
	...
	КонецЕсли; 
	//ИсторияШага.Свойство("ОписаниеОшибки")
	...
	Для каждого СтрСтроки Из Дерево.Строки Цикл
		...
		ИначеЕсли (СтрСтроки.Тип = "Сценарий") или (СтрСтроки.Тип = "Пример") Тогда
			...
			Иначе
				...
				ПрефиксИмениСценария = ПолучитьПрефиксИмениСценария2();
				...
				КонецЕсли;
				СтруктураВыгрузки.historyId = СтруктураВыгрузки.name;
				
				ЗаписатьОписаниеАллюр2(СтрокаФичи, СтруктураВыгрузки);
				ЗаписатьОписаниеАллюр2(СтрСтроки, СтруктураВыгрузки);
												
				СтруктураВыгрузки.Вставить("start", РезультатПрохожденияТестовСценария.ВремяНачала);
				СтруктураВыгрузки.Вставить("stop", РезультатПрохожденияТестовСценария.ВремяОкончания);	
...
Процедура СформироватьОтчетАллюр2(СтруктураОФ = Неопределено, ДеревоТестовПарам, ИмяСборки = "") Экспорт
