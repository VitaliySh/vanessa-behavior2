...
Функция ОпределитьПараметрыВСтрокеПримера(Знач Стр,СтруктураПараметров)
	//Если СтруктураПараметров <> Неопределено Тогда
	//	СтрокаОригинал   = Стр;
	//	Если СтруктураПараметров.Свойство("КешТаблицGherkin") Тогда
	//		КешТаблицGherkin = СтруктураПараметров.КешТаблицGherkin;
	//		СтрокаКешТаблицGherkin = КешТаблицGherkin.Найти(СтрокаОригинал,"Строка");
	//		Если СтрокаКешТаблицGherkin <> Неопределено Тогда
	//			Возврат СтрокаКешТаблицGherkin.СписокПараметров.Скопировать();
	//		КонецЕсли;	 
	//	КонецЕсли;	 
	//КонецЕсли;	 
	...
	//Если СтруктураПараметров <> Неопределено Тогда
	//	Если СтруктураПараметров.Свойство("КешТаблицGherkin") Тогда
	//		СтрокаКешТаблицGherkin                  = КешТаблицGherkin.Добавить();
	//		СтрокаКешТаблицGherkin.Строка           = СтрокаОригинал; 
	//		СтрокаКешТаблицGherkin.СписокПараметров = СписокЗн; 
	//	КонецЕсли;	 
	//КонецЕсли;	 
...
Процедура ФорматТаблицыПримеров(Тзн,РазныеИменованныеПараметры,БылаОшибка,ЭтоПередачаТаблицы,СтруктураПараметров)
	...
	СписокПараметров = ОпределитьПараметрыВСтрокеПримера(Тзн[0].Стр,СтруктураПараметров);
	...
	Для каждого СтрТзн Из Тзн Цикл
		...
		СписокПараметров = ОпределитьПараметрыВСтрокеПримера(СтрТзн.Стр,СтруктураПараметров);
		...
		Для Ккк = 0 По МассивДлин.Количество()-1 Цикл
			Если СписокПараметров.Количество()-1 < Ккк Тогда
				ВызватьИсключение "Неверное число параметров в строке <" + СтрТзн.Стр + ">";
			КонецЕсли;	 
	...
	Для каждого СтрТзн Из Тзн Цикл
		...
		СписокПараметров = ОпределитьПараметрыВСтрокеПримера(СтрТзн.Стр,Неопределено);
		Если СписокПараметров.Количество() = 0 Тогда
			Продолжить;
		КонецЕсли;	 
		...
		Для Каждого ТаблицаСтрокПередачаПараметровТаблицей Из МассивТаблицПередаваемыхКакПараметр Цикл
			...
			Для каждого СтрТаблицаСтрокПередачаПараметровТаблицей Из ТаблицаСтрокПередачаПараметровТаблицей Цикл
				...
				Если СокрЛП(СтрТаблицаСтрокПередачаПараметровТаблицей.Стр) <> "" Тогда
					...
					СтрТаблицаСтрокПередачаПараметровТаблицей.Стр,СтруктураПараметров);
			...
			Если СтрТаблицаШагов.ИмяОбласти <> Неопределено Тогда
				...
				Если СтрТаблицаШагов.НачалоОбласти Тогда
					...
					Если Лев(СтрТаблицаШагов.ИмяОбласти,1) <> "*" Тогда
						...
						//СтараяСтрокаПараметров = "";
						//СтарыйStepDefinition   = ПолучитьStepDefinitionПоСтроке(СтрТаблицаШагов.ИмяШагаБезКлючевогоСлова, ЗначенияПараметров, СтараяСтрокаПараметров,СтрОбласть.ШагСПараметрамиВТаблице,ОпределитьКоличествоПередаваемыхТаблиц(СтрОбласть));
						//НовыйStepDefinition    = ПолучитьНовыйStepDefinition(СтарыйStepDefinition);
...
Функция ПроверитьФичуНаКорректностьСинтаксисаИЗагрузитьСценарии(Знач ИмяФайла,ОписаниеОшибки,ДеревоСтроки,ТаблицаИзвестныхStepDefinition,СтруктураПараметров,МассивСценариевЗащитаОтЗацикливанияКеш,ЭтоЗагрузкаПодчиненногоСценария,ПередаваемыеТаблицы,ИмяЗагружаемогоСценария)
	...
	Для СчетчикСтрокФичи = 1 По КолСтрокТаблицаТекстФичи Цикл
		...
		Если ТекущееКлючевоеСлово = "feature" Тогда
			ОписаниеФичи = "";
			СтруктураПараметров.Вставить("ДопДанныеФичи", Новый Структура);
			СтруктураПараметров["ДопДанныеФичи"].Вставить("name", ВтораяЧастьСтроки);
		...
		ИначеЕсли НадоСтроитьДерево и НачалсяСценарий Тогда
			...
			Если СтрТаблицаТекстФичи.ЗначениеОтступа < СтрТаблицаТекстФичи.СледующийОступ и (СтрТаблицаТекстФичи.НельзяСоздаватьОбласть <> Истина) и (ПроверитьЧтоМожноСоздатьОбласть(ТаблицаТекстФичи,СчетчикСтрокФичи)) Тогда
				...
				ДобавитьШаг(ТаблицаШагов,СтрокаТаблицаТекстФичи,ВтораяЧастьСтроки,"",НомСтр,МассивТаблицПередаваемыхКакПараметр,СруктураОбласти);
...
Функция ПолучитьStepDefinitionПоСтроке(Знач Стр,ЗначенияПараметров, СтрокаПараметров = "",
	...
	//Если ЕстьПараметрыТаблицыПараметр = "ШагСПараметрамиВТаблице" Тогда
	//	ЕстьПараметрыТаблицы = Истина;
	//Иначе
	//	ЕстьПараметрыТаблицы = Ложь;
	//КонецЕсли;	 
	...
	Для Каждого СтрокаКопияТаблицаПеревода Из КопияТаблицаПеревода Цикл
		...
		Если СтрокаКопияТаблицаПеревода.КоличествоРусскихШаговНаОдинСнипетПеревода > 1 Тогда
			...
			Для Каждого СтрокаТаблицаПеревода Из МассивСтрок Цикл
				СтрокаШаг = СтрокаШаг + СтрокаТаблицаПеревода.ОригиналРусскийШаг + Символы.ПС;  
			КонецЦикла;	
...
Функция СделатьПереводСгенерированногоТекстаGherkin(ДанныеПеревода) Экспорт
