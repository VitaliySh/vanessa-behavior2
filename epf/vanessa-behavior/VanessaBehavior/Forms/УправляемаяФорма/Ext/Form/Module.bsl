...
&НаКлиенте
Перем Версия836ИлиВыше;

&НаКлиенте
Перем Версия8311ИлиВыше;
...
&НаКлиенте
Функция УзнатьРежимСовместимостиКлиент()
	Если Не ЗначениеЗаполнено(КешРежимСовместимости) Тогда
		КешРежимСовместимости = УзнатьРежимСовместимости();
	КонецЕсли;	 
...
&НаКлиенте
Функция ПолучитьПутьКОбработкеКлиент()
	Если СокрЛП(Объект.КаталогИнструментов) <> "" Тогда
		Возврат Объект.КаталогИнструментов;
	КонецЕсли;	 
...
Процедура ДобавитьСнипет(Знач Снипет, Знач ИмяФайла, ВерсияФайла, БылиОшибки, ТзнТаблицаИзвестныхStepDefinition)
	...
	Если ПромСтр <> Неопределено Тогда
		БылиОшибки = Истина; 	
		Сообщить("Предупреждение в файле " + ИмяФайла + ", снипет " + ID + " уже был в " + ПромСтр.ИмяФайла);
		//Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьТаблицуКонтекстовОбработокНеПодключаяОбработки(Файл)
	...
	УниверсальноеПолноеИмя = СтрЗаменить(Файл.ПолноеИмя, "\", "/");
...
Процедура ПодключитьСнипетыИзОбработкиКлиент(Файл, ВерсияФайла, БылиОшибки)
	...
	Если ИД = Неопределено Тогда
		БылиОшибки = Ложь;
		Файл = Новый Файл(УниверсальноеПолноеИмя);
		...
		Если МассивИменФайловИзТаблицаИзвестныхStepDefinition <> Неопределено Тогда
			...
			Если ИДМассивИменФайловИзТаблицаИзвестныхStepDefinition <> Неопределено Тогда
				...
				Если МассивВерсийФайловИзТаблицаИзвестныхStepDefinition[ИДМассивИменФайловИзТаблицаИзвестныхStepDefinition] <> ВерсияФайла Тогда
					...
					Отладка("Загрузил новую версию: " + УниверсальноеПолноеИмя);
				Иначе	
					Отладка("Загрузил снипеты из кеш: " + УниверсальноеПолноеИмя);
...
Функция ПолучитьДанныеФайлаПоСобраннойРанееИнформации(ИмяФайла, ДополнительныеПараметры)
	ДанныыеФайла = Неопределено;
	МассивФайлов = Неопределено;
	Для Каждого ЭлементСписка Из Объект.КаталогиБиблиотек Цикл
		МассивФайлов = ПолучитьЗначениеПоРанееСобраннойИнформации(ДополнительныеПараметры, ЭлементСписка.Значение, "НайденныеФайлы"); //это файлы по данной библиотеке
		Если МассивФайлов <> Неопределено Тогда
			Для Каждого ЭлементМассива Из МассивФайлов Цикл
				Если НРег(ЭлементМассива.Файл.ПолноеИмя) = НРег(ИмяФайла) Тогда
					ДанныыеФайла = ЭлементМассива;
					Прервать;
				КонецЕсли;	 
			КонецЦикла;	
		КонецЕсли;	 
	...
	Если ДанныыеФайла = Неопределено Тогда
		МассивФайлов = ПолучитьЗначениеПоРанееСобраннойИнформации(ДополнительныеПараметры, "ПоискEPF", "НайденныеФайлы");
...
Процедура ЗаполнитьДеревоФич(СтруктураПараметров)
	...
	Состояние(НСтр("ru = 'Построение дерева.'; en = 'Building tree.'"));
...
Процедура ПолучитьУжеСуществующиеСценарииИзФич(Знач КаталогФич, ДополнительныеПараметры = Неопределено, МассивУжеОбработанныхаФайлов)
	...
	Для Каждого ФайлОбъект Из МассивФайлов Цикл
		...
		ИмяФайла = СтрЗаменить(Файл.ПолноеИмя, "\", "/");
...
Процедура ЗагрузитьФичиПродолжение(ДополнительныеПараметры = Неопределено)
	...
	Исключение
		...
		Сообщить("Ошибка при построении дерева. <" + ТекущийЗагружаемыйФичаФайл + ">");
	...
	Если НЕ НадоВыполнитьСценарийСоСтрокиПереданнойИзвне Тогда
...
Функция СоздатьСтруктуруПараметровЗапуска()
	...
	Рез.Вставить("ТаймаутЗапуска1С", "5");
...
Процедура ЗапускВРежимеКоманднойСтроки()
	...
	СтрЗапуска = СокрЛП(ПараметрЗапуска);
	//СтрЗапуска = "StartFeaturePlayer;VBParams=C:\Commons\rep\vanessa-behavior\tools\JSON\VBParams8310Web.json";
	Если СтрЗапуска = "" Тогда
		Возврат;
	КонецЕсли;	
...
Функция СформироватьОписаниеОшибки(Знач Стр, ИмяФайла, ИмяПроцедуры) Экспорт
	...
	РежимСовместимости = УзнатьРежимСовместимостиКлиент();
	ТекстИДРаннера     = "";
	Если ЗначениеЗаполнено(Объект.ИДРаннера) Тогда
		ТекстИДРаннера = ", ИДРаннера="+Объект.ИДРаннера;
	КонецЕсли;	 
	
	Стр = Стр + Символы.ПС + "ИмяФайла=" + ИмяФайла + ", ИмяПроцедуры=" + ИмяПроцедуры + ", версия VB=" + ПолучитьВерсиюОбработкиКлиент() + ", версия платформы=" + ВерсияПриложения + ", режим совместимости=" + РежимСовместимости + ТекстИДРаннера;
	Возврат Стр;
КонецФункции
...
Процедура ВыполнитьШаг()
	...
	ИД = МассивИменКонтекстовОбработок.Найти(Нрег(АдресСнипета));
	Если ИД = Неопределено Тогда
		БылиОшибки = Ложь;
	...
	Исключение
		...
		Иначе
			...
			Если Объект.ДелатьЛогВыполненияСценариевВТекстовыйФайл Тогда
				ЗаписатьСтрокуВТекстовыйЛог("Шаг (" + ТекШаг.Имя + ") не выполнен.");
...
Процедура ОпределитьСледующуюСтрокуДереваДляВыполнения(ЭлементыДерево)
	...
	Если ТекРодитель.ДопТип = "Цикл" Тогда
		ЭлементыРодителя = ТекРодитель.ПолучитьЭлементы();
		Если ЭлементыРодителя.Индекс(Элементы.ДеревоТестов.ТекущиеДанные) = ЭлементыРодителя.Количество()-1 Тогда
...
Процедура СделатьСообщениеВсеСценарииОбработаны()
	Если СтатусЗапускаСценариев Тогда
		СделатьСообщение("Выполнение сценариев закончено. Ошибок не было.");
	Иначе	
		СделатьСообщение("Выполнение сценариев закончено. БЫЛИ ОШИБКИ!");
	КонецЕсли;	 
КонецПроцедуры
	...
	Если ИдСледующегоШага <> Неопределено Тогда
		...
		Если ИдСледующегоШага.ИД <> Неопределено Тогда
			Если ИдСледующегоШага.ИскатьПоИдСтроки Тогда
				...
				Если Не НашлиШаг Тогда
					ВызватьИсключение "Не смогли найти следующий шаг.";
				КонецЕсли;	 
...
Процедура ВызватьМетодСценарияЕслиОнЕсть(СтрокаДереваСценария, ИмяПроцедуры, МассивПараметров = Неопределено,
	...
	АдресСнипета = СтрЗаменить(АдресСнипета, "\", "/");
	
	ИДМассивИменКонтекстовОбработок = МассивИменКонтекстовОбработок.Найти(Нрег(АдресСнипета));
...
Функция НайтиМакетВКаталогеПроекта(ИмяМакета) Экспорт
	...
	ИмяФайла = Объект.КаталогПроекта + "\Макеты\" + ИмяМакета + ".mxl";
	Если ФайлСуществуетКомандаСистемы(ИмяФайла) Тогда
		Нашли = Истина;
	КонецЕсли;	 
	
	Если Не Нашли Тогда
		ИмяФайла = Объект.КаталогПроекта + "\" + ИмяМакета + ".mxl";
		Если ФайлСуществуетКомандаСистемы(ИмяФайла) Тогда
			Нашли = Истина;
		КонецЕсли;	 
	КонецЕсли;	 
	...
	Если Не Нашли Тогда
		ВызватьИсключение "Не найден файл макета <" + ИмяМакета + "> в каталоге проекта <" + Объект.КаталогПроекта + ">";
	КонецЕсли;	 
...
&НаКлиенте
Процедура ОбновитьКешДанныхФормыЕслиЭтоНеобходимо(СтрокаСценария,ИД)
	Если СтрокаСценария.ПолучитьРодителя() = Неопределено Тогда
		...
		Если Объект.ЗакрытьTestClientПослеЗапускаСценариев Тогда
			...
			Для каждого СтрокаДанныеКлиентовТестирования Из ДанныеКлиентовТестирования Цикл
				...
				Исключение
					...
					СделатьСообщение("Ошибка закрытия сеанса " +СтрокаДанныеКлиентовТестирования.Имя + ":" +ОписаниеОшибки());
				КонецПопытки;
			КонецЦикла;			
...
&НаКлиенте
Функция ВремяИзмененияФайлаКомандаСистемы(ИмяФайла)
	Если ЕстьПоддержкаАсинхронныхВызовов Тогда
		Если ЭтоLinux Тогда
			//FIXME:
			Возврат ТекущаяДата();
		...
		ИмяФайлаЛога = ПолучитьИмяВременногоФайла("txt");
		//Команда = "dir """ + ИмяФайла + """ /T:W > """ + ИмяФайлаЛога + """";
...
Процедура СоздатьФайлыОбработок(БылиОшибки, СтруктураОписанияEpf, ДополнительныеПараметры = Неопределено)
	...
	Если ФайлСуществуетКомандаСистемы(ИмяВременногоEPF) Тогда
		...
		Если СтруктураОписанияEpf.ФайлEpfПересоздавался Тогда
			СделатьСообщение("Пересоздан " + СтруктураОписанияEpf.ИмяФайлаEpf);
		Иначе	
			СделатьСообщение("Создан " + СтруктураОписанияEpf.ИмяФайлаEpf);
		КонецЕсли;	 
	Иначе
		СделатьСообщение("Файл EPF не создан.");
		СделатьСообщение("Проверьте работу команды <" + СтрокаЗапуска + ">");
		ВывестиТекстФайлаЛога(ИмяФайлаЛога);
		ВывестиТекстФайлаЛога(ИмяФайлаOUT);
	КонецЕсли;
...
Процедура СделатьGenerateEpf(СтруктураПараметров)
	...
	ПолучитьУжеСуществующиеСнипетыИзОбработок(КаталогФич, ДополнительныеПараметры, ДанныеПоВерсиямОбработок.МассивВерсийФайловИзТаблицаИзвестныхStepDefinition, ДанныеПоВерсиямОбработок.МассивИменФайловИзТаблицаИзвестныхStepDefinition, МассивИменФайловКоторыеСовпалиСКеш);
	Для каждого Элем Из КаталогиБиблиотек Цикл
		ПолучитьУжеСуществующиеСнипетыИзОбработок(Элем.Значение, ДополнительныеПараметры, ДанныеПоВерсиямОбработок.МассивВерсийФайловИзТаблицаИзвестныхStepDefinition, ДанныеПоВерсиямОбработок.МассивИменФайловИзТаблицаИзвестныхStepDefinition, МассивИменФайловКоторыеСовпалиСКеш);
	КонецЦикла;
...
Функция СтрокиОжидаемойТаблицыСодержатсяВТаблицеTestclientСервер(ОжидаемаяТаблица,ТаблицаTestClient,СтрокаОшибки)
	...
	Для Каждого Колонка Из Таблица2.Колонки Цикл
		...
		Исключение
			ВызватьИсключение "В таблице не найдена колонка <" + Колонка.Заголовок + ">";
		КонецПопытки;
	КонецЦикла;	
...
&НаКлиенте
Функция НайтиРеквизитОткрытойФормыПоЗаголовку(ИмяРеквизита, ИскатьПоИмени = Ложь,ВызыватьИсключение = Истина, ТипПоля = Неопределено,ДопПараметры = Неопределено) Экспорт
	Нашли = Ложь;
...
Функция ПолучитьПредставлениеДанныхЭлементаФормы(ПолеРеквизит) Экспорт
	...
	Если Тип = Тип("ТестируемоеПолеФормы") Тогда
		//ЭтоВыпадающийСписок = Ложь;
		//Попытка
		//	ПолеРеквизит.ВыпадающийСписокОткрыт();
		//	ЭтоВыпадающийСписок = Истина;
		//Исключение
		//	Сообщить(ОписаниеОшибки());
		//КонецПопытки;
		//
		//Если ЭтоВыпадающийСписок Тогда
...
Функция НайтиТЧПоИмени(ИмяТЧ, НужнаяФорма = Неопределено, ИскатьПоИмени = Истина,ВызыватьИсключение = Истина) Экспорт
	...
	Если НужнаяФорма = Неопределено Тогда
		ВызватьИсключение "Не найдена активная форма.";
	КонецЕсли;	 
	
	Если ТЧ = Неопределено и ВызыватьИсключение Тогда
		Стр = "Таблицы формы с именем <" + ИмяТЧ + "> не найдено.";
		Сообщить(Стр);
		ВызватьИсключение Стр;
	КонецЕсли;	 
...
Функция ПолучитьАктивноеОкноИзТестовоеПриложение(ДопПараметры = Неопределено) Экспорт
	...
	Если НЕ Версия8311ИлиВыше Тогда
		...
		Если ИскатьМодальныйДиалог Тогда
			...
			//указать версию
			ОкноМодальногоДиалога = НайтиОкноПредупреждения();
...
Функция НайтиФормыТекущегоОкнаTestClient(ДопПараметры = Неопределено) Экспорт
	...
	Если ОкноПриложения = Неопределено Тогда
		ВызватьИсключение "Не получилось определить активное окно TestClient.";
	КонецЕсли;	 
...
Функция НайтиОкноПредупреждения() Экспорт
	...
	Если НужноеОкно <> Неопределено Тогда
		ОбъектКонтекст.Вставить("АктивноеОкно",НужноеОкно);
	КонецЕсли;	 
...
Функция НайтиФормуПоЗаголовку(ИмяФормы) Экспорт
	...
	Если НужнаяФорма = Неопределено Тогда
		...
		Для каждого ТекОкно Из МассивОкон Цикл
			...
			Если НужнаяФорма <> Неопределено Тогда
				ОбъектКонтекст.Вставить("АктивноеОкно", ТекОкно); //произошла неявная смена активного окна
				Возврат НужнаяФорма;
			КонецЕсли;	 
...
Процедура ПолучитьПараметрыОбластиМакета(Знач ИмяОбласти,НачСтр,НачКол,КолСтр,КолКол) Экспорт
	...
	Если Поз = 0 Тогда
		ВызватьИсключение "Неверный формат области <" + ИмяОбластиОригинал + ">";
	КонецЕсли;	 
		...
		Если XML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			...
			Если ИмяУзла = "click" Тогда
				Если МассивОбъектов[ИдВМассиве - 1] = "CommandInterfaceButton" Тогда
					Если НРег(МассивСвойств[1]["isMain"]) = "true" Тогда
						...
						ИначеЕсли МассивСвойств[ИдВМассиве - 2]["title"] = "Панель открытых" Тогда
							ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " В панели открытых я выбираю """ + МассивСвойств[ИдВМассиве - 1]["title"] + """");
						ИначеЕсли (МассивСвойств[ИдВМассиве - 2]["title"] = "Меню функций") или (МассивСвойств[ИдВМассиве - 3]["title"] = "Меню функций") Тогда
							...
							Если ТекстРезультат.КоличествоСтрок() > 1 Тогда
								...
								Если Поз > 0 Тогда
									...
									ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " В командном интерфейсе я выбираю " + ИмяПанелиРазделов +  " """ + МассивСвойств[ИдВМассиве - 1]["title"] + """");
								КонецЕсли;	 
							КонецЕсли;	 
				...
				ИначеЕсли МассивОбъектов[ИдВМассиве - 1] = "FormButton" Тогда	
					...
					Если МассивОбъектов[ИдВМассиве - 2] = "FormGroup" Тогда
						Если Прав(МассивСвойств[ИдВМассиве - 2]["name"],15) = "КонтекстноеМеню" Тогда
							ЭтоКонтекстаноеМеню = Истина;
						КонецЕсли;	 
					КонецЕсли;	 
					
					Если ЭтоКонтекстаноеМеню  Тогда
						ЗаголовокОбъекта = МассивСвойств[ИдВМассиве - 3]["title"];
						...
						Если (СокрЛП(ЗаголовокОбъекта) = "") или (ЕстьДублиЗаголовковУПоля(МассивСтруктурФорм, ТекущаяФорма, ЗаголовокОбъекта, "ПолеФормы")) Тогда
							ИмяОбъекта = "с именем """ + МассивСвойств[ИдВМассиве - 3]["name"] + """";
						КонецЕсли;	 
						...
						Если (СокрЛП(ЗаголовокКнопки) = "") или (ЕстьДублиЗаголовковУПоля(МассивСтруктурФорм, ТекущаяФорма, ЗаголовокКнопки, "Кнопка")) Тогда
							ИмяКнопки = "с именем """ + МассивСвойств[ИдВМассиве - 1]["name"] + """";
						КонецЕсли;	 
						
						//ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " у поля " + ИмяОбъекта + " я выбираю пункт контекстного меню " + ИмяКнопки);
			...
			ИначеЕсли ИмяУзла = "gotoDate" Тогда
				ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " у поля """ + МассивСвойств[ИдВМассиве - 1]["title"] + """ я перехожу к дате " + ПреобразоватьСтрокуКДатеИзДанныхXML(МассивСвойств[ИдВМассиве]["date"]));
			ИначеЕсли ИмяУзла = "selectAllRows" Тогда
				ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " в таблице """ + МассивСвойств[ИдВМассиве - 1]["name"] + """ я выделяю все строки");
			ИначеЕсли ИмяУзла = "gotoFirstRow" Тогда
				ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " в таблице """ + МассивСвойств[ИдВМассиве - 1]["name"] + """ я перехожу к первой строке");
			ИначеЕсли ИмяУзла = "gotoLastRow" Тогда
				ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " в таблице """ + МассивСвойств[ИдВМассиве - 1]["name"] + """ я перехожу к последней строке");
			ИначеЕсли ИмяУзла = "deleteRow" Тогда
				ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " в таблице """ + МассивСвойств[ИдВМассиве - 1]["name"] + """ я удаляю текущую строку");
			...
			ИначеЕсли ИмяУзла = "executeChoiceFromChoiceList" Тогда
				Если МассивОбъектов[ИдВМассиве - 1] = "FormField" Тогда
					Если ТекущаяТЧ <> "" Тогда
						...
						Если СокрЛП(ИмяОбъекта) = "" Тогда
							ИмяОбъекта = МассивСвойств[ИдВМассиве - 1]["name"];
							ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " в таблице """ + ТекущаяТЧ + """ из выпадающего списка с именем """ + ИмяОбъекта + """ я выбираю """ + МассивСвойств[ИдВМассиве]["presentation"] + """");
						Иначе	
							ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " в таблице """ + ТекущаяТЧ + """ из выпадающего списка """ + ИмяОбъекта + """ я выбираю """ + МассивСвойств[ИдВМассиве]["presentation"] + """");
						КонецЕсли;	 
					Иначе	
						ИмяОбъекта = МассивСвойств[ИдВМассиве - 1]["title"];
						Если СокрЛП(ИмяОбъекта) <> "" и (Не ЕстьДублиЗаголовковУПоля(МассивСтруктурФорм, ТекущаяФорма, ИмяОбъекта, "ПолеФормы")) Тогда
			...
			ИначеЕсли ИмяУзла = "beginEditingCurrentArea" Тогда
				...
				ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " у табличного документа с именем """ + МассивСвойств[ИдВМассиве - 1]["name"] + """ я делаю двойной клик на текущей ячейке");
			...
			ИначеЕсли ИмяУзла = "setCurrentArea" Тогда
				Если МассивОбъектов[ИдВМассиве - 1] = "FormField" Тогда
					...
					Если ТекстРезультат.КоличествоСтрок() > 0 Тогда
						ПредыдущаяСтрока = ТекстРезультат.ПолучитьСтроку(ТекстРезультат.КоличествоСтрок());
					КонецЕсли;	 
					ТекущаяСтрока = ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " у табличного документа с именем """ + МассивСвойств[ИдВМассиве - 1]["name"] + """ я перехожу к ячейке """ + МассивСвойств[ИдВМассиве]["area"] + """";
			...
			ИначеЕсли ИмяУзла = "inputText" Тогда
				Если МассивОбъектов[ИдВМассиве - 1] = "FormField" Тогда
					...
					Если ФлагРедактированияТекстаМакета Тогда
						ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " у табличного документа с именем """ + МассивСвойств[ИдВМассиве - 2]["name"] + """ я ввожу текст """ + МассивСвойств[ИдВМассиве]["text"] + """");
					Иначе	
						ЗаголовокПоля = МассивСвойств[ИдВМассиве - 1]["title"];
						Если ТекущаяТЧ <> "" Тогда	
							Если СокрЛП(ЗаголовокПоля) <> "" Тогда
								ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " в таблице """ + ТекущаяТЧ + """ в поле """ + МассивСвойств[ИдВМассиве - 1]["title"] + """ я ввожу текст """ + МассивСвойств[ИдВМассиве]["text"] + """");
							Иначе	
								ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " в таблице """ + ТекущаяТЧ + """ в поле с именем """ + МассивСвойств[ИдВМассиве - 1]["name"] + """ я ввожу текст """ + МассивСвойств[ИдВМассиве]["text"] + """");
							КонецЕсли;	 
						Иначе	
			...
			ИначеЕсли ИмяУзла = "setCheck" Тогда
				Если МассивОбъектов[ИдВМассиве - 1] = "FormField" Тогда
					...
					Если ТекущаяТЧ <> "" Тогда		
						Если СокрЛП(МассивСвойств[ИдВМассиве - 1]["title"]) <> "" Тогда
							ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " в таблице """ + ТекущаяТЧ + """ я изменяю флаг """ + МассивСвойств[ИдВМассиве - 1]["title"] + """");
						Иначе	
							ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " в таблице """ + ТекущаяТЧ + """ я изменяю флаг с именем """ + МассивСвойств[ИдВМассиве - 1]["name"] + """");
						КонецЕсли;	 
					Иначе	
						Если СокрЛП(МассивСвойств[ИдВМассиве - 1]["title"]) <> "" Тогда
			...
			Если (ИмяУзла = "gotoRow") или (ИмяУзла = "expand") или (ИмяУзла = "collapse") Тогда
				...
				Если ТаблицаСвойствСтроки.Количество() > 0 Тогда
					Если (ИмяУзла = "gotoRow") Тогда
						ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " в таблице """ + МассивСвойств[ИдВМассиве - 1]["name"] +  """ я перехожу к строке:");
					ИначеЕсли (ИмяУзла = "expand") Тогда
						ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " в таблице """ + МассивСвойств[ИдВМассиве - 1]["name"] +  """ я разворачиваю строку:");
					ИначеЕсли (ИмяУзла = "collapse") Тогда
						ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " в таблице """ + МассивСвойств[ИдВМассиве - 1]["name"] +  """ я сворачиваю строку:");
					КонецЕсли;	 
					ТекстРезультат.ДобавитьСтроку(Стр);
				Иначе	
					Если (ИмяУзла = "expand") Тогда
						ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " в таблице """ + МассивСвойств[ИдВМассиве - 1]["name"] +  """ я разворачиваю текущую строку");
					КонецЕсли;	
				КонецЕсли;	 
...
Функция ПроверитьПортНаЗанятость(Знач Порт = 1538) Экспорт
	...
	Если ЗначениеЗаполнено(Объект.ДиапазонПортовTestclient) Тогда
		...
		Если МассивЗначений.Количество() <> 2 Тогда
			ВызватьИсключение "Указан неверный диапазон портов: <" + Объект.ДиапазонПортовTestclient + ">";
		КонецЕсли;	 
...
Процедура УстановитьКонектСTestClient(ДанныеЗапускаКлиентаТестирования, ТестовоеПриложение)
	...
	Пока Истина Цикл
		Если ТекущаяДата() > МаксДата Тогда
			ВызватьИсключение "Не сомог подключить Testclient. Прервывание по таймауту <"
...
Функция ОткрытьНовыйСеансTestClientИлиПодключитьУжеСуществующийДляНесколькихКлиентов(ДанныеЗапускаКлиентаТестирования,СтрокаОшибки=Неопределено)
	...
	Исключение
		...
		СтрокаОшибки = "Сеанс должен быть запущен с ключом TESTMANAGER!";
	    Возврат Ложь;
	КонецПопытки;
...
Процедура ДобавитьВнешнийСписокПроцедур(МассивСнипетовИзОбработки, ПолноеИмя, ФормаОбработки) Экспорт 
	...
	УниверсальноеПолноеИмя = СтрЗаменить(ПолноеИмя, "\", "/");
	МассивИменКонтекстовОбработок.Добавить(Нрег(УниверсальноеПолноеИмя));
	БылиОшибки = Ложь;
	Для каждого Снипет из МассивСнипетовИзОбработки Цикл 
...
Процедура ДеревоТестовПриАктивизацииСтроки(Элемент)
	...
	Если ТекущиеДанные.Тип = "Фича" Тогда
	...
	ИначеЕсли ТекущиеДанные.Тип = "Сценарий" Тогда
...
Функция МакетTestClientПоЯчейкеКакМассив(ЭлементФормы,ДопПараметры = Неопределено)
	...
	Пока Истина Цикл
		...
		Пока Истина Цикл
			...
			Если НЕ ТекущийTestclientЭтоWebКлиент Тогда
...
Функция ПолучитьЗначениеТестируемаяТаблицаФормы(ЭлементФормы) Экспорт
	Если ЭлементФормы.ТекущийРежимРедактирование() Тогда
		Попытка
	...
	ИначеЕсли ТипЗнч(ЭлементФормы) = Тип("ТестируемоеПолеФормы") Тогда
		...
		Если ЭлементФормы.Вид = ВидПоляФормы.ПолеТабличногоДокумента Тогда
			Попытка
				...
				КонецЕсли;	 
			Исключение
				Сообщить("Не смог получить значение табличного документа <" + ЭлементФормы.Имя + ">");
				Сообщить(ОписаниеОшибки());
			КонецПопытки;
...
Функция ПолучитьСостояниеФормыTestClient()
	...
	Попытка
		...
		АктивноеОкно = НайтиОкноПредупреждения();
		...
		Сообщить("Найдена форма: ТекстЗаголовка=" + ТекущаяФорма.ТекстЗаголовка + ", ИмяФормы=" + ТекущаяФорма.ИмяФормы + ". Окно " + "<" + АктивноеОкно.Заголовок + ">");
	Исключение
	...
	Для каждого ЭлемСтало Из Стало.МассивЭлементов Цикл
		...
		Для каждого Свойство Из ЭлемСтало Цикл
			...
			Если Ключ = "ПредставлениеДанных" Тогда
				...
				Если Было = Неопределено Тогда
					Изменение = "элемент формы с именем """ + ЭлемСтало.Имя + """ стал равен """ + ПредставлениеДанных + """";
					...
					Если ЭлемСтало.Свойство("ТекстРедактирования") Тогда
						Если ЭлемСтало.ТекстРедактирования <> ПредставлениеДанных Тогда
							Изменение = "у элемента формы с именем """ + ЭлемСтало.Имя + """ текст редактирования стал равен """ + ЭлемСтало.ТекстРедактирования + """";
							Стр = Стр + "	И     " + Изменение + Символы.ПС;
						КонецЕсли;	 
					КонецЕсли;	 
				...
				Иначе	
					...
					Если ЭлемСтало.Свойство("ТекстРедактирования") Тогда
						...
						Если СокрЛП(ЭлемСтало["ТекстРедактирования"]) <> СокрЛП(ЭлемБыло["ТекстРедактирования"]) Тогда
							Изменение = "у элемента формы с именем """ + ЭлемСтало.Имя + """ текст редактирования стал равен """ + ЭлемСтало.ТекстРедактирования + """";
...
Процедура ИсследовательФормы(Команда)
	...
	ФормаДобавитьШаг = ПолучитьФорму("ВнешняяОбработка.VanessaBehavior.Форма.ИсследовательФормы", ПараметрыФормы,ЭтаФорма);
...
Процедура СделатьСкриншотыОшибкиКаждогоОкна1СSikuliX()
	...
	СделатьСообщение("Активирую окно с координатами: " + XMLСтрока(ТекКоординаты.x) + ";" + XMLСтрока(ТекКоординаты.y));
	ВыполнитьSikuliСкрипт(Объект.КаталогИнструментов + "/tools/Sikuli/ClickCoordinates.sikuli --args " + XMLСтрока(ТекКоординаты.x) + " " + XMLСтрока(ТекКоординаты.y) , -1, Истина);
...
Процедура СделатьСкриншотыОшибки()
	Если Объект.СниматьСкриншотКаждогоОкна1С Тогда
		...
		СделатьСообщение("Найдено " + МассивКоординатОкон1С.Количество() + " окон.");
...
Процедура УстановитьРезультатУсловия(Результат) Экспорт
	Если ТипЗнч(Результат) <> Тип("Булево") Тогда
		ВызватьИсключение "В процедуру <УстановитьРезультатУсловия> передан неверный тип параметра <" + Результат + ">. Ожидаемый тип: Булево.";
	КонецЕсли;	 
...
Функция ПолучитьСтрокуЗапускаWebКлиент(ДанныеЗапускаКлиентаТестирования)
	СтрокаЗапуска = ДанныеЗапускаКлиентаТестирования.ПутьКИнфобазе;
	...
	МассивДоппараметров = РазложитьСтрокуВМассивПодстрок(ДанныеЗапускаКлиентаТестирования.ДопПараметры," ");
	Для Каждого Элем Из МассивДоппараметров Цикл
		Если Лев(Элем,2) = "/N" Тогда
			ДобавитьПараметрКWeb(СтрокаЗапуска, "N", Сред(Элем,3));
		КонецЕсли;	 
		Если Лев(Элем,2) = "/P" Тогда
			ДобавитьПараметрКWeb(СтрокаЗапуска, "P", Сред(Элем,3));
		КонецЕсли;	 
	КонецЦикла;	
...
Функция ПолучитьСтрокуЗапускаDesktopПриложение(ДанныеЗапускаКлиентаТестирования)
	...
	Если СвободныйПорт <> ДанныеЗапускаКлиентаТестирования.ПортЗапускаТестКлиента Тогда
		СделатьСообщение("Порт " + ДанныеЗапускаКлиентаТестирования.ПортЗапускаТестКлиента + " занят или находится вне указанного диапазона, порт подключения изменен на " + СвободныйПорт);
...
Процедура ЗапуститьКлиентаТестирования(ДанныеЗапускаКлиентаТестирования, ДопПараметр = "")
	...
	ИначеЕсли Не ЗначениеЗаполнено(ДанныеЗапускаКлиентаТестирования.ТипКлиента) Или
		 Найти(НРег(ДанныеЗапускаКлиентаТестирования.ТипКлиента), "тонкий") > 0
		 Тогда
...
Процедура ДобавитьИзвестныйШаг(Команда)
	...
	ФормаДобавитьШаг = ПолучитьФорму("ВнешняяОбработка.VanessaBehavior.Форма.ВыборИзвестногоШага", ПараметрыФормы,ЭтаФорма);
	ФормаДобавитьШаг.Открыть();
КонецПроцедуры
...
Процедура ПодключитьНайденныеКлиентскиеПлагиныНаСервере(Знач ЗагруженныеФайлы, Знач ИдентификаторыПлагинов)
	...
	Перем ОбъектПлагина, ОписанияПлагиновКлиент, ПодключенныеРанее; 
	...
	Для каждого ЗагруженныйФайл Из ЗагруженныеФайлы Цикл
		...
		Попытка 
			ОбъектПлагина = ПолучитьФорму("ВнешняяОбработка." + ИмяПлагина + ".Форма", , ЭтаФорма);
			ОбъектПлагина.Инициализация(ЭтаФорма);
			ОписанияПлагиновКлиент.Вставить(ЗагруженныйФайл.Имя, ОбъектПлагина.ОписаниеПлагина(Объект.ТипыПлагинов));
			ПодключенныеРанее.Вставить(ЗагруженныйФайл.Имя, Истина);
			Сообщить("Подключен плагин " + ИмяПлагина + " в режиме отладки");
		Исключение
	...
	Для Каждого ЗагруженныйФайл Из ЗагруженныеФайлы Цикл
		...
		Если ОписаниеПлагина = Неопределено Тогда
			ОписаниеПлагина = ОписанияПлагиновКлиент.Получить(ЗагруженныйФайл.Имя);
		КонецЕсли;
...
Функция ПодключитьПлагиныНаСервере(Знач ЗагружаемыеФайлы, Знач ПодключенныеРанее)
	...
	Для Каждого ОписаниеФайла Из ЗагружаемыеФайлы Цикл
		...
		Если ПодключенныеРанее.Получить(ОписаниеФайла.Имя) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ПодключитьФайлОбработкиНаСервере(Знач АдресХранилища, Знач ИдентификаторОбработки, ЭтоОтчет = Ложь)
	ПодключитьВнешнююОбработкуСервер(АдресХранилища);
КонецПроцедуры
...
Функция СинхронныйРежимРазрешен()
	Если мСинхронныйРежимРазрешен = Неопределено Тогда
		мСинхронныйРежимРазрешен = СинхронныйРежимРазрешенНаСервере();
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция СинхронныйРежимРазрешенНаСервере()
	
	СистемнаяИнформация = Новый СистемнаяИнформация;
	Если ВерсияПриложенияБольшеИлиРавнаЧемЗаданная(СистемнаяИнформация.ВерсияПриложения, "8.3.10.1") Тогда
		Возврат Вычислить("Метаданные.РежимИспользованияСинхронныхВызововРасширенийПлатформыИВнешнихКомпонент <> Метаданные.СвойстваОбъектов.РежимИспользованияСинхронныхВызововРасширенийПлатформыИВнешнихКомпонент.НеИспользовать");
	Иначе
		Возврат Вычислить("Метаданные.РежимИспользованияСинхронныхВызововРасширенийИВнешнихКомпонент <> Метаданные.СвойстваОбъектов.РежимИспользованияСинхронныхВызововРасширенийИВнешнихКомпонент.НеИспользовать");
	КонецЕсли;
...
&НаКлиенте
Процедура СохранитьЗначениеВКонтекстСохраняемый(ИмяПеременной,Значение) Экспорт
...
&НаКлиенте
Процедура УстановитьФлагОстановкиВыполненияШагов() Экспорт
	ПрекратитьВыполнениеВсехСценариев = Истина;
КонецПроцедуры
...
Процедура СделатьПереводСгенерированногоТекстаGherkinСервер(ДанныеПеревода)
	...
	FeatureReader.СделатьПереводСгенерированногоТекстаGherkin(ДанныеПеревода);
...
Процедура СделатьПереводТекстаGherkin()
	...
	//ИмяФайлаПеревода = Объект.КаталогИнструментов + "\locales\Gherkin\" + Объект.ЯзыкГенератораGherkin + ".mxl"; 
	//
	//Если Не ФайлСуществуетКомандаСистемы(ИмяФайлаПеревода) Тогда
	//	ВызватьИсключение "Не найден файл <" + ИмяФайлаПеревода + ">";
	//КонецЕсли;	 
	...
	ДанныеПеревода.Вставить("ТекстДляПеревода",Объект.СгенерированныйСценарий);
	ДвДанныеvbFeatureReader = Новый ДвоичныеДанные(ПолучитьПутьКFeatureReader());
	...
	Если ДанныеПеревода.Свойство("Перевод") Тогда
		Если ЗначениеЗаполнено(ДанныеПеревода.Перевод) Тогда
			Объект.СгенерированныйСценарий = ДанныеПеревода.Перевод;
		КонецЕсли;	 
	КонецЕсли;	 
КонецПроцедуры
...
Процедура ВыполнитьКодНаКлиенте(Команда)
	...
	ТекстДляВыполнения = Объект.СгенерированныйСценарийКод;
	Попытка
