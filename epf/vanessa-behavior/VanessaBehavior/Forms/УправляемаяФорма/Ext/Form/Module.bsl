...
&НаКлиенте
Функция МакетTestClientПоЯчейке(ЭлементФормы)
	ТабДок = Новый Массив();
...
&НаСервере
Функция ПреобразоватьМакетВМассивСогласноПараметровСчитывания(ФайлМакета)
...
&НаСервере
Функция ПреобразоватьМакетВМассив(ФайлМакета)
	ИмяВременногоФайла = ПолучитьИмяВременногоФайла("mxl");
...
Функция ПолучитьТабДокИзTestClient(ЭлементФормы) Экспорт
	...
	Если НЕ ТекущийTestclientЭтоWebКлиент И ВерсияПриложенияБольшеИлиРавнаЧемЗаданная(СистемнаяИнформация.ВерсияПриложения, "8.3.8.0") Тогда
		...
		Если Не ФайлСуществуетКомандаСистемы(ИмяВременногоФайла) Тогда
			...
			Возврат ТабличныйДокументTestClientПоЯчейкам(ЭлементФормы);
		КонецЕсли;	 
		...
		//ФайлМакета = Новый ДвоичныеДанные(ИмяВременногоФайла);
		ТабДок = ПреобразоватьМакетВМассив(ФайлМакета);
		УбратьПустыеСтрокиИКолонкиСКраёвМакета(ТабДок);
		Возврат ТабДок;
	Иначе	
		//значит надо считывать макет по ячейке
		Возврат МакетTestClientПоЯчейке(ЭлементФормы);
	КонецЕсли;	 
КонецФункции	

&НаСервере
Функция ОставитьЧастьМакетаСогласноПараметровЧтенияМакета(ФайлМакета)
	ИмяВременногоФайла = ПолучитьИмяВременногоФайла("mxl");
