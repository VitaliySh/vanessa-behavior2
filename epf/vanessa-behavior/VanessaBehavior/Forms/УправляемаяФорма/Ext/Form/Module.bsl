...
Процедура ВосстановитьНастройкиКлиент() Экспорт
	...
	Настройки.ВосстановитьНастройки(Объект);
...
Процедура ЗапускВРежимеКоманднойСтроки()
	...
	Настройки = Плагин("LoadSettings");
	СтруктураДействий = Новый Структура("ВыполнитьСценарии, ВыполнитьСценарииСТекущегоШага, ЗавершитьРаботуСистемы, НомерСтрокиДляЗапускаСТекущегоШагаСценария, ЗагрузитьСценарии");
...
Процедура ПодключитьНайденныеКлиентскиеПлагины(Знач ЗагруженныеФайлы, Знач ИдентификаторыПлагинов)
	...
	Для каждого ОбъектПлагина Из МассивПодключенныхПлагинов Цикл
		...
		Исключение
			СделатьСообщение("Ошибка подключения плагина "+ОбъектПлагина.Заголовок + ":" + ОписаниеОшибки());
		КонецПопытки;
