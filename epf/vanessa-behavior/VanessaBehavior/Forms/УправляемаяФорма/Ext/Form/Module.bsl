...
Функция СформироватьОписаниеОшибки(Знач Стр, ИмяФайла, ИмяПроцедуры) Экспорт
	...
	Попытка
		...
		Если ЗначениеЗаполнено(ИнфоОбОшибке) Тогда
			ТекстОшибкиTestClient = ", ИнформацияОбОшибке=" + ИнфоОбОшибке;
		КонецЕсли;	 
	Исключение
	КонецПопытки;
	
	Стр = Стр + Символы.ПС + "ИмяФайла=" + ИмяФайла + ", ИмяПроцедуры=" + ИмяПроцедуры + ", версия VB=" + ПолучитьВерсиюОбработкиКлиент() + ", версия платформы=" + ВерсияПриложения + ", режим совместимости=" + РежимСовместимости + ТекстИДРаннера + ТекстОшибкиTestClient;
	Возврат Стр;
КонецФункции
...
Функция НайтиФормыОкнаИзКонтекста(ДопПараметры = Неопределено) Экспорт
	ОкноПриложения         = ПолучитьАктивноеОкноИзКонтекста();
	Если ОкноПриложения = Неопределено Тогда
		ВызватьИсключение "Не получилось определить активное окно TestClient из Контекста.";
	КонецЕсли;	 
	МассивФорм             = ОкноПриложения.НайтиОбъекты(Тип("ТестируемаяФорма"));
	Возврат МассивФорм;
КонецФункции	
...
Функция ОткрытьНовыйСеансTestClientИлиПодключитьУжеСуществующийДляНесколькихКлиентов(ДанныеЗапускаКлиентаТестирования,СтрокаОшибки=Неопределено)
	...
	Попытка
		...
		Если НЕ ТестовоеПриложение.ОжидатьОтображениеОбъекта(Тип("ТестируемоеОкноКлиентскогоПриложения"),,,Объект.ТаймаутЗапуска1С) Тогда
			ВызватьИсключение "Не дождались отображения окна TestClient. Таймаут <" + Объект.ТаймаутЗапуска1С + ">";
		КонецЕсли;	 
