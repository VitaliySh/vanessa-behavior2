	...
	Для каждого СтрСтроки Из Дерево.Строки Цикл
		...
		ИначеЕсли (СтрСтроки.Тип = "Сценарий") или (СтрСтроки.Тип = "Пример") Тогда
			...
			Иначе
				...
				ДобавитьПараметр(Фабрика, СписокПараметров, "argument", 			"Версия1С", ""+Строка(СисИнформация.ВерсияПриложения)); 
				ДобавитьПараметр(Фабрика, СписокПараметров, "argument", "ТипПлатформы", 		""+Строка(СисИнформация.ТипПлатформы) + "; РежимЗапуска:"+ТекущийРежимЗапуска());
				ДобавитьПараметр(Фабрика, СписокПараметров, "argument", 			"СтрокаСоединенияИБ", Строка(СтрокаСоединенияИнформационнойБазы()));
				ДобавитьПараметр(Фабрика, СписокПараметров, "argument", 			"Теги", 	"Отбор:"+ЭтотОбъект.СписокТеговОтбор + "; Исключение:"+ЭтотОбъект.СписокТеговИсключение);
