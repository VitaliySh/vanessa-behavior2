			...
			Иначе
				...
				Если СтрСтроки.Родитель.Тип = "Фича" Тогда
					...
					Если ТипЗнч(ДополнительноеОписаниеФичи) = Тип("Структура") И ДополнительноеОписаниеФичи.Свойство("tags") Тогда
						МассивТэгов = ДополнительноеОписаниеФичи["tags"];
					КонецЕсли;
				...
				Для каждого Тэг из МассивТэгов Цикл
					ДобавитьМетку(Фабрика, СписокМеток, "tag", Тэг, СписокМетокПереопределяемый);
				КонецЦикла;
				...
				ДобавитьПараметр(Фабрика, СписокПараметров, , 			"Версия1С", ""+Строка(СисИнформация.ВерсияПриложения)); 
				ДобавитьПараметр(Фабрика, СписокПараметров, , "ТипПлатформы", 		""+Строка(СисИнформация.ТипПлатформы) + "; РежимЗапуска:"+ТекущийРежимЗапуска());
				ДобавитьПараметр(Фабрика, СписокПараметров, , 			"СтрокаСоединенияИБ", Строка(СтрокаСоединенияИнформационнойБазы()));
				ДобавитьПараметр(Фабрика, СписокПараметров, , 			"Теги", 	"Отбор:"+ЭтотОбъект.СписокТеговОтбор + "; Исключение:"+ЭтотОбъект.СписокТеговИсключение);
