...
Процедура ДобавитьПараметр(Фабрика, СписокПараметров, ТипПараметра = Неопределено, name, value)
	Если ТипПараметра = Неопределено Тогда
		ТипПараметра = "environment-variable";
	КонецЕсли;

	Типparameter   = Фабрика.Тип("urn:model.allure.qatools.yandex.ru", "parameter");
	Параметр       = Фабрика.Создать(Типparameter);
	Параметр.name  = name;
	Параметр.value = Строка(value);
	Параметр.kind  = ТипПараметра;

	СписокПараметров.parameter.Добавить(Параметр);
...
Процедура ОбойтиДеревоДляОтчетаАллюр(Дерево,Фабрика,ТекущаяФича = Неопределено,СписокСценариев = Неопределено,СписокШагов = Неопределено)
	Для каждого СтрСтроки Из Дерево.Строки Цикл
		Если      СтрСтроки.Тип = "Фича" Тогда
			...
			ДополнительноеОписаниеФичи = СтрСтроки.ДополнительныеДанные;
			Если ТипЗнч(ДополнительноеОписаниеФичи) = Тип("Структура") И ДополнительноеОписаниеФичи.Свойство("description") Тогда
				Типdescription   = Фабрика.Тип("urn:model.allure.qatools.yandex.ru", "description");
				Описание = Фабрика.Создать(Типdescription);
				Описание.type = "markdown";
				Описание.__content = ДополнительноеОписаниеФичи["description"]; 

				ТекущаяФича.description= Описание;
			КонецЕсли;
			...
			Для каждого ЭлементМетки из СписокМетокПереопределяемый Цикл
				ДобавитьМетку(Фабрика, СписокМеток, ЭлементМетки.Значение, ЭлементМетки.Представление);
			КонецЦикла;
				
			ДобавитьМетку(Фабрика,СписокМеток,"framework","vanessa-behavior", СписокМетокПереопределяемый);
			ДобавитьМетку(Фабрика,СписокМеток,"language","1С", СписокМетокПереопределяемый);
		...
		ИначеЕсли (СтрСтроки.Тип = "Сценарий") или (СтрСтроки.Тип = "Пример") Тогда
			...
			Иначе
				...
				Если СтрСтроки.Родитель.Тип = "Фича" Тогда
					ДополнительноеОписаниеФичи = СтрСтроки.Родитель.ДополнительныеДанные;
				...
				ДобавитьМетку(Фабрика, СписокМеток, "story", ЗначениеМеткиStory, СписокМетокПереопределяемый); 
				ДобавитьМетку(Фабрика, СписокМеток, "feature",СтрСтроки.Родитель.Имя, СписокМетокПереопределяемый);
				ДобавитьМетку(Фабрика, СписокМеток, "host", СисИнформация.ВерсияОС, СписокМетокПереопределяемый);
				
				ФайлФичи = Новый Файл(СтрСтроки.Родитель.ПолныйПуть);
				ПутьФичи = Новый Файл(ФайлФичи.Путь);
				КаталогФичи = ПутьФичи.ИмяБезРасширения;
				ДобавитьМетку(Фабрика, СписокМеток, "package", КаталогФичи, СписокМетокПереопределяемый);
				...
				СписокПараметров = Фабрика.Создать(Типparameters);
				//Для allure и allure2 дублируем в аргументах и основные параметры.
				ДобавитьПараметр(Фабрика, СписокПараметров, "argument", "ОС", СисИнформация.ВерсияОС);
				ДобавитьПараметр(Фабрика, СписокПараметров, "argument", 			"Версия1С", СисИнформация.ВерсияПриложения);
				ДобавитьПараметр(Фабрика, СписокПараметров, "environment-variable", "Версия1С", СисИнформация.ВерсияПриложения);
				ДобавитьПараметр(Фабрика, СписокПараметров, "argument", "ТипПлатформы", 		СисИнформация.ТипПлатформы);
				ДобавитьПараметр(Фабрика, СписокПараметров, "argument", "ТекущийРежимЗапуска", 	ТекущийРежимЗапуска());
				ДобавитьПараметр(Фабрика, СписокПараметров, "argument", 			"СтрокаСоединенияИБ", СтрокаСоединенияИнформационнойБазы());
				ДобавитьПараметр(Фабрика, СписокПараметров, "argument", 			"ТегиОтбора", 	ЭтотОбъект.СписокТеговОтбор);
				ДобавитьПараметр(Фабрика, СписокПараметров, "argument", 			"ТегиИсключения", ЭтотОбъект.СписокТеговИсключение);				
				ДобавитьПараметр(Фабрика, СписокПараметров, "environment-variable", "СтрокаСоединенияИБ", СтрокаСоединенияИнформационнойБазы());
				ДобавитьПараметр(Фабрика, СписокПараметров, "environment-variable", "ТегиОтбора", 	ЭтотОбъект.СписокТеговОтбор);
				ДобавитьПараметр(Фабрика, СписокПараметров, "environment-variable", "ТегиИсключения", ЭтотОбъект.СписокТеговИсключение);				
				
				Сценарий.parameters = СписокПараметров;
...
Процедура ОбойтиДеревоДляОтчетаCucumberJson(Дерево, ЗаписьJson, ДопПараметры,ДанныеФичи = Неопределено)
	Для каждого СтрСтроки Из Дерево.Строки Цикл
		Если      СтрСтроки.Тип = "Фича" Тогда
			...
			ЗаписьJSON.ЗаписатьЗначение(Транслит(СтрСтроки.Имя));
			
			ЗаписьJSON.ЗаписатьИмяСвойства("keyword");
			ЗаписьJSON.ЗаписатьЗначение("Feature");
			
			ЗаписьJSON.ЗаписатьИмяСвойства("id");
			ПолныйПуть = СтрЗаменить(СтрСтроки.ПолныйПуть, КаталогФич, "");
			
			Если СтрДлина(ПолныйПуть) = 0 Или СтрДлина(ПолныйПуть) > 29 Тогда
				ПолныйПуть = Сред(СтрСтроки.ПолныйПуть, СтрДлина(СтрСтроки.ПолныйПуть) - 29);
			КонецЕсли;
			
			ЗаписьJSON.ЗаписатьЗначение(Транслит(ПрефиксИмениСценария + ПолныйПуть));
...
Функция СравнитьТаблицы(ТаблицаОжиданий, ТаблицаРезультатов, ТаблицаРазличий,ДопСообщениеОшибки = "")
...
Процедура ПроверитьРавенствоТаблиц(Таб1, Таб2, ДопСообщениеОшибки = "") Экспорт
	...
	Различия = Новый ТаблицаЗначений;
	РезультатСравнения = СравнитьТаблицы(Таб1, Таб2, Различия,ДопСообщениеОшибки);
